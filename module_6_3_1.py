...
class Pegasus(Hourse, Eagle): # класс описывающий пегаса, классы родители: Horse и Eagle
    ...
    def __init__(self): # метод __init__ для class Pegasus
        Hourse.__init__(self) # обращение к классу Hourse, для видимости первого родительского класса
    ...
    def move(self, dx, dy): # метод move, dx и dy изменения дистанции. В этом методе должны запускаться наследованные
       ...
       # return self.fly(dy), self.run(dx) # возврат наследованных методов run и fly. При таком методе
       # решения сдесь этот возврат только удваивает значения dx и dy
...
p1.move(10, 15) # изменения дистанции dx и dy
